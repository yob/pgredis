...
func (cmd *getCommand) Execute(command redisProtoCommand, redis *PgRedis, writer *redisproto.Writer) error {
...
func (cmd *incrCommand) Execute(command redisProtoCommand, redis *PgRedis, writer *redisproto.Writer) error {
	key := command.Get(1)
	newValue, err := redis.strings.Incr(key, 1)
	if err == nil {
		intValue, _ := strconv.Atoi(string(newValue))
